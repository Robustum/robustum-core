...
internal class KeyDispatchCodec<K : Any, V : Any>(
    val typeKey: String,
    val keyCodec: Codec<K>,
    val type: (V) -> DataResult<K>,
    val decoder: (K) -> DataResult<out MapDecoder<out V>>,
    ...
    override fun <T : Any> decode(ops: DynamicOps<T>, input: MapLike<T>): DataResult<V> {
        ...
        return keyCodec.decode(ops, elementName).flatMap { type1: Pair<K, T> ->
            decoder(type1.first).flatMap { elementDecoder: MapDecoder<out V> ->
                if (ops.compressMaps()) {
                    input
